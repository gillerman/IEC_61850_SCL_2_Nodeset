...
-->
    ...
    <xsl:for-each select="//nsSCL:Substation">
      <xsl:for-each select="nsSCL:VoltageLevel">
        <UAObject>
          ...
          <xsl:element name="Description"><xsl:value-of select="@desc"/> Voltage Level</xsl:element>
          <References>
        ...
        <UAObject>
          ...
          <References>
            ...
            <xsl:for-each select="nsSCL:LNode">
              <Reference ReferenceType="ns=1;s=Parent.LN" IsForward="true">ns=1;s=<xsl:value-of select="@ldInst"/>
                <xsl:value-of select="@lnClass"/><xsl:value-of select="@lnInst"/></Reference>
            </xsl:for-each> 
          </References>
        </UAObject>
        <xsl:for-each select="nsSCL:TransformerWinding">
          ...
          <xsl:for-each select="nsSCL:Terminal">
            <xsl:call-template name="ProcessTerminal">
              <xsl:with-param name="PSSElement"><xsl:value-of select="../@name"/></xsl:with-param>
    ...
    <xsl:for-each select="//nsSCL:Substation/nsSCL:VoltageLevel">
      <xsl:for-each select="nsSCL:Bay">
        ...
        <xsl:for-each select="nsSCL:ConductingEquipment">
          <UAObject>
            <xsl:attribute name="NodeId">ns=1;s=<xsl:value-of select="@name"/><xsl:value-of select="@type"/></xsl:attribute>
            <xsl:attribute name="BrowseName">1:<xsl:value-of select="@name"/><xsl:value-of select="@type"/></xsl:attribute>
            <xsl:element name="DisplayName"><xsl:value-of select="@name"/> <xsl:value-of select="@type"/> Conducting Equipment</xsl:element>
      ...
      <xsl:for-each select="nsSCL:ConnectivityNode">
        <xsl:call-template name="ProcessConnectivityNode">
          <xsl:with-param name="PPSSElement"><xsl:value-of select="../@name"/></xsl:with-param>
          <xsl:with-param name="PPconnectivityNode"><xsl:value-of select="@connectivityNode"/></xsl:with-param>
          <xsl:with-param name="PPcNodeName"><xsl:value-of select="@cNodeName"/></xsl:with-param>
        </xsl:call-template>
      </xsl:for-each>
    </xsl:for-each>       
    ...
    <xsl:for-each select="//nsSCL:Substation/nsSCL:VoltageLevel/nsSCL:Bay/nsSCL:ConductingEquipment">
      <xsl:for-each select="nsSCL:Terminal">
        <xsl:call-template name="ProcessTerminal">
          <xsl:with-param name="PSSElement"><xsl:value-of select="../@name"/><xsl:value-of select="../@type"/></xsl:with-param>
          <xsl:with-param name="PconnectivityNode"><xsl:value-of select="@connectivityNode"/></xsl:with-param>
          <xsl:with-param name="PcNodeName"><xsl:value-of select="@cNodeName"/></xsl:with-param>
        </xsl:call-template>
      </xsl:for-each>
      <xsl:for-each select="nsSCL:SubEquipment">
        <UAObject>
          <xsl:attribute name="NodeId">ns=1;s=<xsl:value-of select="@name"/><xsl:value-of select="@phase"/></xsl:attribute>
          <xsl:attribute name="BrowseName">1:<xsl:value-of select="@name"/><xsl:value-of select="@phase"/></xsl:attribute>
          <xsl:element name="DisplayName"><xsl:value-of select="@name"/> <xsl:value-of select="@phase"/> Phase Sub Equipment</xsl:element>
          <xsl:element name="Description"><xsl:value-of select="@name"/> <xsl:value-of select="@phase"/> Phase Sub Equipment</xsl:element>
          <References>
            <Reference ReferenceType="HasComponent" IsForward="false">ns=1;s=<xsl:value-of select="../@name"/><xsl:value-of select="../@type"/></Reference>
    ...
    <xsl:for-each select="//nsSCL:IED">
      <xsl:for-each select="nsSCL:AccessPoint">
        <UAObject>
          ...
          <xsl:element name="DisplayName"><xsl:value-of select="../@name"/>IED/<xsl:value-of select="@name"/> Access Point</xsl:element>
    ...
    <xsl:for-each select="//nsSCL:IED/nsSCL:AccessPoint">
      <xsl:for-each select="nsSCL:Server">
        <UAObject>
          ...
          <xsl:element name="DisplayName"><xsl:value-of select="../../@name"/>IED/<xsl:value-of select="../@name"/>AP/Server</xsl:element>
...
<!-- Aux Templates-->
  ...
  <xsl:template name="ProcessTerminal">
    <xsl:param name="PSSElement"/>
    <xsl:param name="PconnectivityNode"/>
	  <xsl:param name="PcNodeName"/>
    <UAObject>
      <xsl:attribute name="NodeId">ns=1;s=<xsl:value-of select="$PSSElement"/>/<xsl:value-of select="$PconnectivityNode"/></xsl:attribute>
      <xsl:attribute name="BrowseName">1:<xsl:value-of select="$PSSElement"/>/<xsl:value-of select="$PconnectivityNode"/></xsl:attribute>
      <xsl:element name="DisplayName"><xsl:value-of select="$PSSElement"/>/<xsl:value-of select="$PconnectivityNode"/> Terminal</xsl:element>
      <xsl:element name="Description"><xsl:value-of select="$PSSElement"/>/<xsl:value-of select="$PconnectivityNode"/> Terminal</xsl:element>
      <References>
        <Reference ReferenceType="HasComponent" IsForward="false">ns=1;s=<xsl:value-of select="$PSSElement"/></Reference>
        ...
        <Reference ReferenceType="ns=1;s=T.CN" IsForward="true">ns=1;s=<xsl:value-of select="$PcNodeName"/></Reference>
      </References>
    </UAObject>
    <xsl:call-template name="ProcessConnectivityNode">
      <xsl:with-param name="PPSSElement"><xsl:value-of select="$PSSElement"/></xsl:with-param>
      <xsl:with-param name="PPconnectivityNode"><xsl:value-of select="$PconnectivityNode"/></xsl:with-param>
      <xsl:with-param name="PPcNodeName"><xsl:value-of select="$PcNodeName"/></xsl:with-param>
    </xsl:call-template>
  </xsl:template>
  
  <xsl:template name="ProcessConnectivityNode">
    <xsl:param name="PPSSElement"/>
    <xsl:param name="PPconnectivityNode"/>
	  <xsl:param name="PPcNodeName"/>
    <xsl:for-each select="distinct-values($PPcNodeName)">
      <UAObject>
        <xsl:attribute name="NodeId">ns=1;s=<xsl:value-of select="$PPcNodeName"/></xsl:attribute>
        <xsl:attribute name="BrowseName">1:<xsl:value-of select="$PPcNodeName"/></xsl:attribute>
        <xsl:element name="DisplayName"><xsl:value-of select="$PPcNodeName"/> Connectivity Node</xsl:element>
        <xsl:element name="Description"><xsl:value-of select="$PPcNodeName"/> Connectivity Node</xsl:element>
        <References>
  ...
  <xsl:template name="ProcessLN">
    ...
    <UAObject>
			...
			<xsl:element name="DisplayName"><xsl:value-of select="$PLN"/> Logical Node</xsl:element>
